...
int main()
{
    ...
    bool success = Dll_SetUserTimeRestriction(WIN_USER, WEEK_DAYS, TIME_INTERVAL);
    if (!success)
    {
    ...
    {
        cout << endl
             << "Computing and distributing points for F=" << fValue << "... ";
        ...
        if (!success)
        {
            ...
            if (!success) printProgramError("Dll_DeleteWorkingFolderTree", "Couldnt run command"); 
            return 1;
        }
    ...
    cout << endl
         << "Execution time: " << totalTime << " s" << endl;
